...
const AddHorseImage = (props: Props) => {
    ...
    return (
        <UploadControlImages id="general_upload" path={`/horses/${name}`} profile={props.profile}  >
            <div className={`border-double border-4 border-white ${hovering ? "border-opacity-40" : "border-opacity-20"} flex z-20 w-full relative h-52 transition-all hover:filter-none bg-gray-600`} onMouseLeave={() => setHovering(false)} onMouseEnter={() => setHovering(true)} >
